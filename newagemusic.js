// ==UserScript==
// @name New Age Music
// @trackerURL https://newage-music.ru
...
// @description Инструментальная и электронная музыка нового века.
// @downloadURL https://raw.githubusercontent.com/hkhiuo/newage-music/34186fd5cf53b35bc11cb3bd39262214fead6686/newagemusic.js
// @connect *://newage-music.ru/*
// @require jquery
// @version 1.0.1
// ==/UserScript==
...
var onPageLoad = function (response) {
    ...
    if (/login\.php/.test(response.url) || $bodyDom.find('#login-form').length) {
        return {success: false, error: 'AUTH', message: 'requireAuth', url: 'https://newage-music.ru/login.php'};
    }
...
API_event('search', function(request) {
    return API_request({
        method: 'POST',
        url: 'https://newage-music.ru/tracker.php',
        data: {
            nm: request.query
        }
    }).then(onPageLoad);
